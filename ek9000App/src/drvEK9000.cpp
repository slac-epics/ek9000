...
drvEK9000::drvEK9000(const char* ek, const char* port, const char* ipport, const char* ip)
	: drvModbusAsyn(port, ipport, 0, 2, -1, 65535, dataTypeUInt16, 0, ""), name(ek) {
...
void drvEK9000::StartPollThread() {
	...
	this->pollThread =
		epicsThreadCreate(name, epicsThreadPriorityHigh, epicsThreadStackBig, drvEK9000::PollThreadFunc, (void*)this);
}
...
void drvEK9000::PollThreadFunc(void* lparam) {
	...
	while (1) {
		...
		if (dt > _this->pollPeriod * 1000.0f) {
			asynPrint(_this->pasynUserSelf, ASYN_TRACE_WARNING,
					  "%s:%s: Total time between poll calls exceeds poll period! dt=%f, period=%f", function, step, dt,
					  (_this->pollPeriod * 1000.0f));
		}
		...
		if (_this->doModbusIO(0, MODBUS_READ_INPUT_REGISTERS, INPUT_PDO_START, _this->inputSwapSpace, 120) !=
				asynSuccess ||
			_this->doModbusIO(0, MODBUS_READ_INPUT_REGISTERS, INPUT_PDO_START + 120, &_this->inputSwapSpace[120],
							  120) != asynSuccess ||
			_this->doModbusIO(0, MODBUS_READ_INPUT_REGISTERS, INPUT_PDO_START + 240, &_this->inputSwapSpace[240], 15) !=
				asynSuccess) {
			asynPrint(_this->pasynUserSelf, ASYN_TRACE_ERROR,
					  "%s:%s Failed to read input registers, skipping to input bits\n", function, step);
		}

		/* Read input bits */
		step = "ReadInputBits";
		if (_this->doModbusIO(0, MODBUS_READ_DISCRETE_INPUTS, INPUT_PDO_START, _this->inputBitSwap, 120) !=
				asynSuccess ||
			_this->doModbusIO(0, MODBUS_READ_DISCRETE_INPUTS, INPUT_PDO_START + 120, &_this->inputBitSwap[120], 120) !=
				asynSuccess ||
			_this->doModbusIO(0, MODBUS_READ_DISCRETE_INPUTS, INPUT_PDO_START + 240, &_this->inputBitSwap[240], 15) !=
				asynSuccess) {
		...
		step = "WriteOutputRegisters";
		if (_this->doModbusIO(0, MODBUS_WRITE_MULTIPLE_REGISTERS, OUTPUT_PDO_START, _this->outputSwapSpace, 120) !=
				asynSuccess ||
			_this->doModbusIO(0, MODBUS_WRITE_MULTIPLE_REGISTERS, OUTPUT_PDO_START + 120, &_this->outputSwapSpace[120],
							  120) != asynSuccess ||
			_this->doModbusIO(0, MODBUS_WRITE_MULTIPLE_REGISTERS, OUTPUT_PDO_START + 240, &_this->outputSwapSpace[240],
							  15) != asynSuccess) {
			asynPrint(_this->pasynUserSelf, ASYN_TRACE_ERROR,
					  "%s:%s Failed to write output registers, skipping to output bits.\n", function, step);
		}

		/* Do I/O for the coils */
		step = "WriteOutputCoils";
		if (_this->doModbusIO(0, MODBUS_WRITE_MULTIPLE_COILS, OUTPUT_COIL_START, _this->outputBitSwap, 120) !=
				asynSuccess ||
			_this->doModbusIO(0, MODBUS_WRITE_MULTIPLE_COILS, OUTPUT_COIL_START + 120, &_this->outputBitSwap[120],
							  120) != asynSuccess ||
			_this->doModbusIO(0, MODBUS_WRITE_MULTIPLE_COILS, OUTPUT_COIL_START + 240, &_this->outputBitSwap[240],
							  15) != asynSuccess) {
		...
		while (1) {
			...
			if (coe_dt >= (_this->pollPeriod * 1000.0f)) {
				asynPrint(_this->pasynUserSelf, ASYN_TRACE_WARNING,
						  "%s: %s: Poll period exceeded for missed due to CoE requests or other latency. coe_dt=%d\n",
...
void drvEK9000::MapTerminals() {
	...
	for (int i = 0; terminalids[i] != 0 && i < 0xFF; i++) {
		...
		/* Determine the type */
		if (terminalids[i] >= 3000 && terminalids[i] < 4000)
			terminals[i].type = terminal_t::AI;
		if (terminalids[i] >= 4000 && terminalids[i] < 5000)
			terminals[i].type = terminal_t::AO;
		if (terminalids[i] >= 1000 && terminalids[i] < 2000)
			terminals[i].type = terminal_t::BI;
		if (terminalids[i] >= 2000 && terminalids[i] < 3000)
			terminals[i].type = terminal_t::BO;
 ...
 * rw = false means read
 */
bool drvEK9000::doCoEIO(bool rw, epicsUInt16 termid, epicsUInt16 index, epicsUInt16 subindex, epicsUInt16 bytesize,
						epicsUInt16* pbuf) {
	assert(bytesize <= 250);
	assert(pbuf != NULL);
	...
	/* Read */
		...
		status =
			this->doModbusIO(0, MODBUS_WRITE_MULTIPLE_REGISTERS, REG_0x1400, send_data, 0x5 /* Number of registers */);
...
bool drvEK9000::doCoEIO(coe_req_t req) {
	return this->doCoEIO(req.type == COE_REQ_WRITE, req.termid, req.index, req.subindex, BYTES_TO_REG(req.length),
						 req.pdata);
}
...
void ek9k_register() {
	util::iocshRegister("ek9000Register", ek9000Register,
						{{"ek9k-name", iocshArgString}, {"port", iocshArgString}, {"ip", iocshArgString}});
}
